...
def upsidedown(
    ...
    """
    This checks whether the robot is flipped over (upside-down) by
    evaluating gravity direction in the robot's local body frame.
    """
    data = env.scene[asset_cfg.name].data
...
def contact(
    ...
    return torch.any(
        torch.max(torch.norm(net_contact_forces[:, :, asset_cfg.body_ids], dim=-1), dim=1)[0] > 1.0,
        dim=1,
    )
...
def n_foot_contact(
    ...
    """
    Ensure that exactly n feet are
    in contact with the ground only
    when the robot is moving fast
    enough.
    """
    contact_sensor = env.scene[asset_cfg.name]
    net_contact_forces = contact_sensor.data.net_forces_w_history
...
def action_rate(
    ...
    """
    This constraint limits how fast the actions (joint targets) are allowed to change over time. This helps:
        - Prevent overly aggressive joint commands.

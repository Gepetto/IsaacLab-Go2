...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_mean = nn.Sequential(
            ...
            layer_init(nn.Linear(256, 128)),
            nn.ELU(),
            layer_init(nn.Linear(128, single_action_space), std=0.01),
        )
...
class RobotModel(nn.Module):
    def __init__(self, envs):
        ...
        self.sequential = nn.Sequential(
            layer_init(nn.Linear(single_observation_space + robot_torques_size, 512)),
            nn.ELU(),
            layer_init(nn.Linear(512, 256)),
            nn.ELU(),
            layer_init(nn.Linear(256, 128)),
            nn.ELU(),
            layer_init(nn.Linear(128, single_state_size), std=0.01),
        )
...
def PPO(envs, ppo_cfg, run_path):
    ...
    robot_model = RobotModel(envs).to(device)
    robot_model_optimizer = optim.RAdam(robot_model.parameters(), lr=LEARNING_RATE, eps=1e-5)
    robot_model_loss_fn = torch.nn.MSELoss()
    ...
    pd_targets = torch.zeros((NUM_STEPS, NUM_ENVS, 24), requires_grad=True, dtype=torch.float).to(device)
    actual_states = torch.zeros((NUM_STEPS, NUM_ENVS, 24), requires_grad=True, dtype=torch.float).to(device)
    ...
    for iteration in range(1, NUM_ITERATIONS + 1):
        ...
        for step in range(0, NUM_STEPS):
            ...
            with torch.no_grad():
                ...
                pd_target = robot_model(next_obs, action)
                values[step] = value.flatten()
            pd_targets[step] = pd_target
            actions[step] = action
            logprobs[step] = logprob

            # TRY NOT TO MODIFY: execute the game and log data.
            next_obs, rewards[step], next_done, timeouts, info = envs.step(torch.cat([pd_target, action], -1))
            next_done = next_done.to(torch.float)
            next_obs = next_obs["policy"]
            # Extract position and velocity states
            actual_states[step] = next_obs[:, 9 : 9 + 24]
        ...
        b_pd_targets = pd_targets.reshape(-1)
        b_actual_states = actual_states.reshape(-1)

        robot_model_optimizer.zero_grad()
        robot_model_loss = robot_model_loss_fn(b_pd_targets, b_actual_states)
        robot_model_loss.backward(retain_graph=True)
        robot_model_optimizer.step()
        writer.add_scalar("Loss/robot_model", robot_model_loss.item() / NUM_STEPS, iteration)
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(NUM_STEPS)):
                ...
                advantages[t] = lastgaelam = (
                    delta + GAMMA * GAE_LAMBDA * nextnonterminal * true_nextnonterminal * lastgaelam
                )

...
def joint_position(
    ...
    robot = env.scene[asset_cfg.name]  # noqa F841
...
def action_rate(
    ...
    robot = env.scene[asset_cfg.name]  # noqa F841
    data = env.scene[asset_cfg.name].data  # noqa F841
    return (
        torch.abs(
...
def base_height(env: ManagerBasedRLEnv, low_bound: int, asset_cfg: SceneEntityCfg):
    """ """
    asset = env.scene[asset_cfg.name]
    return low_bound - asset.data.root_pos_w[:, 2]
